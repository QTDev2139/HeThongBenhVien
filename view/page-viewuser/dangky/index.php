...
<head>
    ...
    <style>
        /* Giảm kích thước các phần tử trong form */
        .form-control {
            font-size: 0.875rem;
            padding: 0.5rem;
        }
            ...
            if ($last_id = $obj->addBenhNhan($sql)) {
                ...
                if ($last_id) {
                    ...
                    $sql2= "INSERT INTO HoaDon (maThuNgan, maBenhNhan, NgayLap, TongTien, TrangThaiThanhToan, NgayThanhToan, LoaiHoaDon)
                            VALUES 
                            (NULL, $last_id, CURDATE(), 500000, 'Đã Thanh Toán', CURDATE(), 'Khám bệnh');
                            ";
                    $obj2->addHD($sql2);
    
                    echo "<script>alert('Mã bệnh nhân của bạn là: $last_id');</script>";
                    header("refresh:0.5; url='index.php?page=thanhtoan&cate=" . urlencode($last_id) . "&cate1=" . urlencode($maCa) . "&cate2=" . urlencode($sdt) . "'");
                }
            } else {
